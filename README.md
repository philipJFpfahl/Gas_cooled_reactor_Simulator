...
so that $$\rho = \rho_{cr}+\rho_{T}+\rho_{Xe} $$
with
$$
...
$$
and the control roods being Sigmoidal:
$$
\rho_{cr} = 4000/(1+exp(-x10))-2000
 $$
 with the steady state at the controllrod position $x = 0$


The Xenon concentration $Xe$ is calculated using the flux $$ \Phi = \frac{P}{\Sigma_f Q_{fission}} $$
and the usual:
...
The entire power of the reactor is assumed to be dumped into the coolant gas. At some point it might be interesting to test radiate cooling.
...
The heat flow into the battery the difference of the in and outlet temperature (incopressible). So that 
$$
...
We assume that the efficiency is 50% of the carnot effici